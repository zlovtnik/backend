...
```rust
// Good: Combinator chain (email: Option<String>)
...
fn find_user_email_bad(users: &[User], id: u64) -> Option<String> {
    match users.iter().find(|u| u.id == id) {
        Some(user) => match user.email.as_ref() {
...
async fn fetch_all(urls: Vec<String>) -> Vec<Result<String>> {
    ...
    for handle in handles {
        match handle.await {
            Ok(inner) => results.push(inner), // inner is Result<String, anyhow::Error>
            Err(join_err) => results.push(Err(anyhow::anyhow!("task failed: {join_err}"))),
        }
    }
    results
}
