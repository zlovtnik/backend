...
use crate::{
    ...
    models::nfe_document::{
        operations as nfe_ops, validators as nfe_validators, NewNfeDocument, NfeDocument,
        UpdateNfeDocument,
    },
...
pub fn new_nfe_validator() -> Validator<NewNfeDocument> {
    Validator::new().rule(nfe_validators::validate_new_nfe)
}

/// Validator for updating NFE documents
pub fn update_nfe_validator() -> Validator<UpdateNfeDocument> {
    Validator::new().rule(nfe_validators::validate_update_nfe)
}
...
pub fn delete_nfe_reader(document_id: i32) -> QueryReader<usize> {
    QueryReader::new(move |conn| {
        nfe_ops::delete_nfe_document(document_id, conn).map_err(|e| match e {
